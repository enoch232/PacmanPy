...
def depthFirstSearch(problem):
    ...
    closed = []
    fringe = util.Stack()
    fringe.push([Node(problem.getStartState(), "" , 1)])
    while 1:
        current_path = fringe.pop()
        node = current_path[len(current_path)-1]
        if problem.isGoalState(node.getState()):
            return_direction = []
            for each in current_path:
                return_direction.append(each.getAction())
            return return_direction[1:]
        if node.getState() not in closed:
            closed.append(node.getState())
            successors = problem.getSuccessors(node.getState())
            for successor in successors:
                new_path = current_path[:]
                new_path.append(Node(successor[0], successor[1], successor[2]))
